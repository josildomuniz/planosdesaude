...
def whatsapp_webhook():
    ...
    print(f"Estado atual para {sender_number}: '{current_state}'")

    # Tenta converter a mensagem de entrada para inteiro APENAS SE for relevante
    # para a tomada de decisão em um estado que espera números.
    # Caso contrário, mantemos como string.
    try:
    ...
    # Esta é a condição de mais alta prioridade. Se for acionada, ela define o estado
    # para 'main_menu' e encerra o processamento para esta requisição.
    if incoming_msg in ["olá", "oi", "menu", "voltar", "iniciar"] or \
       current_state == 'initial' or \
       current_state == 'finished':
        
        user_states[sender_number] = 'main_menu'
        user_data[sender_number] = {} # Sempre limpa dados ao iniciar ou reiniciar
    ...
    # Esta seção só é alcançada se a condição acima não foi satisfeita.
    # A ordem dos `elif` aqui é importante, mas o `current_state` é o que realmente direciona.
    ...
    # --- Caminho 2: Dúvidas ---
    # --- ESTADO: doubt_type --- (Qual tipo de dúvida?)
    ...
    # --- Caminho 3: Suporte ---
    # --- ESTADO: support_type --- (Qual tipo de suporte?)
    ...
    # --- ESTADO: other_subject_description --- (Coleta da descrição de "Outro assunto")

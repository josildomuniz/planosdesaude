...
# Nome do arquivo para persistir os estados e dados
...
# --- Funções para Carregar/Salvar Estados e Dados ---
...
# Carrega os estados e dados ao iniciar o aplicativo
# Estas variáveis globais serão carregadas uma vez.
# Em produção, você usaria um banco de dados de verdade.
user_states = load_user_data(STATE_FILE)
user_data = load_user_data(DATA_FILE)
print(f"INFO: Estados carregados: {user_states}")
print(f"INFO: Dados carregados: {user_data}")
...
def whatsapp_webhook():
    ...
    # Carrega o estado e dados do usuário específicos para esta requisição (do arquivo)
    # Garante que user_states e user_data sejam as últimas versões persistidas.
    global user_states, user_data # Declara que estamos usando as variáveis globais
    user_states = load_user_data(STATE_FILE)
    user_data = load_user_data(DATA_FILE)
    ...
    print(f"Estado atual para {sender_number}: '{current_state}' (do arquivo)")
    ...
    if incoming_msg in ["olá", "oi", "menu", "voltar", "iniciar"] or \
       ...
       current_state == 'finished':
        ...
        user_data[sender_number] = {} # Sempre limpa dados anteriores para um novo fluxo
    ...
    # --- Caminho 1: Cotação ---
        ...
        print(f"DEBUG: Dados coletados para cotação individual/família de {sender_number}: {user_data[sender_number]}")
    ...
    # --- Fluxo PME (ramificação da Cotação) ---
        ...
        print(f"DEBUG: Dados coletados para cotação PME de {sender_number}: {user_data[sender_number]}")
    ...
    # --- Caminho 2: Dúvidas ---
        ...
        print(f"DEBUG: Dados coletados para dúvida de cobertura de {sender_number}: {user_data[sender_number]}")
        ...
        print(f"DEBUG: Dados coletados para dúvida de reajuste/valores de {sender_number}: {user_data[sender_number]}")
        ...
        print(f"DEBUG: Dados coletados para dúvida de carências de {sender_number}: {user_data[sender_number]}")
        ...
        print(f"DEBUG: Dados coletados para dúvida de como usar o plano de {sender_number}: {user_data[sender_number]}")
        ...
        print(f"DEBUG: Dados coletados para outra dúvida de {sender_number}: {user_data[sender_number]}")
    ...
    # --- Caminho 3: Suporte ---
        ...
        print(f"DEBUG: Dados coletados para falar com especialista de {sender_number}: {user_data[sender_number]}")
        ...
        print(f"DEBUG: Dados coletados para agendamento de reunião de {sender_number}: {user_data[sender_number]}")
        ...
        print(f"DEBUG: Dados coletados para problema com boleto de {sender_number}: {user_data[sender_number]}")
    ...
    # --- Caminho 4: Outro Assunto ---
        ...
        print(f"DEBUG: Dados coletados para outro assunto de {sender_number}: {user_data[sender_number]}")
    ...
    # --- SALVA OS ESTADOS E DADOS APÓS CADA REQUISIÇÃO ---
    save_user_data(user_states, STATE_FILE)
    save_user_data(user_data, DATA_FILE)
    print(f"DEBUG: Estados salvos: {user_states}")
    print(f"DEBUG: Dados salvos: {user_data}")
